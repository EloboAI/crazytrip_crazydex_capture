...
use crate::models::{Capture, DeviceUpload, AnalysisResult};
...
impl DatabaseService {
    ...
    pub async fn init_schema(&self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        // Indexes
        client.execute("CREATE INDEX IF NOT EXISTS idx_captures_user_id ON captures(user_id)", &[]).await?;
        client.execute("CREATE INDEX IF NOT EXISTS idx_captures_created_at ON captures(created_at DESC)", &[]).await?;
        client.execute("CREATE INDEX IF NOT EXISTS idx_captures_category ON captures(category)", &[]).await?;
        client.execute("CREATE INDEX IF NOT EXISTS idx_captures_device_local_id ON captures(device_local_id)", &[]).await?;
        client.execute("CREATE INDEX IF NOT EXISTS idx_captures_is_deleted ON captures(is_deleted)", &[]).await?;
        ...
        client.execute("CREATE INDEX IF NOT EXISTS idx_device_uploads_status ON device_uploads(status)", &[]).await?;
        ...
        client.execute("CREATE INDEX IF NOT EXISTS idx_analysis_queue_status ON analysis_queue(status)", &[]).await?;
    ...
    /// Create a capture
    pub async fn create_capture(&self, req: &crate::models::CreateCaptureRequest) -> Result<Capture, Box<dyn std::error::Error + Send + Sync>> {
        let client = self.get_client().await?;
        ...
        let row = client.query_one("
            INSERT INTO captures (id, user_id, device_local_id, image_url, thumbnail_url, image_size, 
    ...
    /// Get capture by ID
    pub async fn get_capture_by_id(&self, id: &Uuid) -> Result<Option<Capture>, Box<dyn std::error::Error + Send + Sync>> {
        let client = self.get_client().await?;
    ...
    pub async fn get_captures(&self, user_id: Option<Uuid>, page: i32, limit: i32) -> Result<(Vec<Capture>, i64), Box<dyn std::error::Error + Send + Sync>> {
        let client = self.get_client().await?;
    ...
    /// Update capture
    pub async fn update_capture(&self, id: &Uuid, req: &crate::models::UpdateCaptureRequest) -> Result<Option<Capture>, Box<dyn std::error::Error + Send + Sync>> {
        let client = self.get_client().await?;
        let now = Utc::now();
    ...
    /// Soft delete capture
    pub async fn delete_capture(&self, id: &Uuid) -> Result<bool, Box<dyn std::error::Error + Send + Sync>> {
        let client = self.get_client().await?;
    ...
    pub async fn get_pending_analysis(&self, limit: i32) -> Result<Vec<Uuid>, Box<dyn std::error::Error + Send + Sync>> {
        let client = self.get_client().await?;
    ...
    pub async fn mark_analysis_completed(&self, capture_id: &Uuid) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        let client = self.get_client().await?;
    ...
    fn row_to_capture(row: &tokio_postgres::Row) -> Capture {
        Capture {
            id: row.get(0),
            user_id: row.get(1),
            device_local_id: row.get(2),
            image_url: row.get(3),
            thumbnail_url: row.get(4),
            image_size: row.get(5),
            storage_type: row.get(6),
            vision_result: row.get(7),
            category: row.get(8),
            confidence: row.get(9),
            tags: row.get(10),
            location: row.get(11),
            location_info: row.get(12),
